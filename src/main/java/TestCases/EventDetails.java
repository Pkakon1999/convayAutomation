...
import Page_Objects.EventDetails_Page;
...
public class EventDetails {
	...
	@Test(priority = 1) // Test case to access event details from Scheduler
	void Event_Details() throws InterruptedException {
	    // Navigate to the Manage User page after login
	    EventDetails_Page event = new EventDetails_Page(driver);

	    // Click on OpenScheduler
	    event.clickScheduler();
	    Thread.sleep(3000);

	    // Click on Event Details
	    event.clickEventDetails();
	    Thread.sleep(3000);

	    // Get the scrollable card container
	    JavascriptExecutor js = (JavascriptExecutor) driver;
	    WebElement cardContainer = event.getCardContainer();
