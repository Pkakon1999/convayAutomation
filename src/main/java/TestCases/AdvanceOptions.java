...
import Page_Objects.AdvanceOptions_Page;
...
public class AdvanceOptions {
	...
	@Test(priority = 1) // Test case to access Advance Options from Scheduler
	void Advance_Options() throws InterruptedException {
	    // Navigate to the Manage User page after login
	    AdvanceOptions_Page options = new AdvanceOptions_Page(driver);

	    // Click on OpenScheduler
	    options.clickScheduler();
	    Thread.sleep(3000);

	    // Click on Advance Options
	    options.clickAdvanceOptions();
	    Thread.sleep(3000);

	    // Get the scrollable card container
	    JavascriptExecutor js = (JavascriptExecutor) driver;
	    WebElement cardContainer = options.getCardContainer();
