...
import Page_Objects.AddParticipants_Page;
...
public class AddParticipants {
	...
	@Test(priority = 1) // Test case to Add Participant from Scheduler
	void Add_Participant() throws InterruptedException {
		// Navigate to the Manage User page after login
		AddParticipants_Page add = new AddParticipants_Page(driver);
		...
		WebElement cardContainer = add.getCardContainer();
		...
		// Click on Event Details
		add.clickAddParticipant();
		Thread.sleep(2000);

		// Click on Add textbox
		add.clickAdd();
		Thread.sleep(2000);
