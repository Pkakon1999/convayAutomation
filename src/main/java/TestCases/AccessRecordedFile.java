...
import Page_Objects.AccessRecordedFile_Page;
import Utilities.Take_Screenshot;

public class AccessRecordedFile {
	...
	@Test(priority = 1) // Test case to access recorded file (Copy)
	void Recorded_File_Copy() throws InterruptedException {
		// Navigate to the Start meeting option after login
		AccessRecordedFile_Page accessRecord = new AccessRecordedFile_Page(driver);

		// Click on meeting management
		accessRecord.clickMeetingManagement();
		Thread.sleep(5000);

		// Click on previous meeting
		accessRecord.clickPreviousMeeting();
		Thread.sleep(5000);

		// Select a meeting to access recordings
		accessRecord.selectMeeting();
		Thread.sleep(2000);

		// Click on Recordings tab
		accessRecord.clickRecordings();
		Thread.sleep(2000);

		// Click on Copy button
		accessRecord.clickCopy();
		Thread.sleep(2000);
		
		String getToasterValue = accessRecord.getToasterValue();

		// Verify the toaster message text
		Assert.assertEquals(getToasterValue, "Record link copied to clipboard");
	...
	@Test(priority = 2) // Test case to access recorded file (Download)
	void Recorded_File_Download() throws InterruptedException {
		// Navigate to the Start meeting option after login
		AccessRecordedFile_Page accessRecord = new AccessRecordedFile_Page(driver);
