...
import Page_Objects.AcceptInvitation_Page;
...
public class AcceptInvitation {
	...
	@Test(priority = 1) // Tect case to Accept invitation
	public void CancelInvitation() throws InterruptedException {
		AcceptInvitation_Page accept = new AcceptInvitation_Page(driver);
	    
	    // Click on Accept button from invitation card
		accept.clickAcceptInvitation();
		Thread.sleep(2000);
		...
		String getToasterValue = accept.getToasterValue();

		// Verify the toaster message text
		Assert.assertEquals(getToasterValue, "Accepted the meeting Mosharof Hossain's scheduled meeting");
	}
