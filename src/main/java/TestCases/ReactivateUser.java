...
import Page_Objects.ReactivateUser_Page;
...
public class ReactivateUser {
	...
	@Test(priority = 1) // Test case to Reactivate a user
	void FilterByStatus_Active() throws InterruptedException {
		// Navigate to the Add User page after login
		ReactivateUser_Page reactivateUserPage = new ReactivateUser_Page(driver);

		// Open the Admin dropdown
		reactivateUserPage.clickAdminDropdown();
		Thread.sleep(2000);

		// Click on "Manage Users"
		reactivateUserPage.clickManageUsers();
		Thread.sleep(2000);

		// Click on "Search User"
		Thread.sleep(2000);
		reactivateUserPage.roleDropdown();
		Thread.sleep(2000);

		// FIlter suspended user from dropdown
		reactivateUserPage.selectSuspended();
		Thread.sleep(2000);

		// Click on 3 dot for which user you want to suspend
		reactivateUserPage.select3dot();
		Thread.sleep(2000);

		// Click on Reactivate
		reactivateUserPage.selectReactivate();
		Thread.sleep(2000);

		// Click on Confirm Activation
		reactivateUserPage.selectConfirmActivation();
		Thread.sleep(2000);

		String getToasterValue = reactivateUserPage.getToasterValue();
	...
	@Test(priority = 3) // Test case to check the reactivate user can login or not
